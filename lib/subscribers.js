...
export async function upsertSubscriberRecord({ email, status, errorMessage }) {
  // Best-effort local ledger. On read-only filesystems (Vercel, etc.) the
  // disk write will fail — that must not block the Substack sync, which is
  // the actual signup. Log and move on.
  try {
    ...
    console.warn(
      "[subscribers] local ledger write failed (continuing):",
